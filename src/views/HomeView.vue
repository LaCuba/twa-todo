...
<template>
  <div class="min-h-full bg-gray-900 text-white p-6">
    ...
    <form @submit.prevent="addTodo" class="flex mb-6">
      <input
        v-model="newTodo"
        class="flex-1 p-3 rounded-l bg-gray-800 text-white border border-gray-700"
      ...
      >
        <div class="flex items-center space-x-3 w-full cursor-pointer">
          ...
          <div v-if="editingId === todo.id" class="flex-1">
            <input
              v-model="editText"
              @keydown.enter.prevent="saveEdit(todo.id)"
          ...
          <button
            ...
            class="text-red-400 hover:text-red-300 ml-3 cursor-pointer"
          >
            ✕
          </button>
        </div>
